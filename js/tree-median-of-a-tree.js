...
which takes the root of the Binary Search Tree as input and returns the Median of Node values in the given BST.
...
class Solution {
    ...
    findMedian(root) {
        //code here
        const arr = this.getData(root);
        const len = arr.length;
        const mid = Math.floor(len / 2);
        let result = arr[mid];
        if ((len & 1) == 0) { //even length
            result = Math.floor(arr[mid] + arr[mid - 1]) / 2;
        }
        return result;
    }
    ...
    getData(root) {
        if (root == null) return;
        if (root.left != null) this.getData(root.left);
        this.ans.push(root.data);
        if (root.right != null) this.getData(root.right);
        return this.ans;
    }
}